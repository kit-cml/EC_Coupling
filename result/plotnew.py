...
plt.scatter(time2, data2['cai'].head(len(time2)), color='blue', label='Scaled')
plt.scatter(time1, data1['cai'].head(len(time1)), color='lightblue', label='Not Scaled')
plt.scatter(data_main['Time'], data_main['Cai_1000msec']*0.000001, color='red', label='Control')
...
# Add legend
plt.title("Calcium Concentration (in mMol) - 1000 BCL")
